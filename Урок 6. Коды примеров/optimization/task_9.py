...
class BasicClass:
    __slots__ = ('param_x', 'param_y', 'param_z') #меняю способ хранения атрибутов обьекта (на кортеж здесь)
    def __init__(self, param_x, param_y):
        self.param_x = param_x
        self.param_y = param_y
        self.param_z = 'z_parameter'

BC_OBJ = BasicClass(5, 6)
print(BC_OBJ.__slots__)
print(asizeof.asizeof(BC_OBJ))
#BC_OBJ.param_z = 7 # работать не будет класс со слотами  так не дает атрибут создавать
print(BC_OBJ.__slots__)
